...
def generate_K_R(fov, theta, phi, height, width, order='yx'):
    ...
    rot_vec1 = np.ascontiguousarray((x_axis * np.radians(phi)).astype(np.float64).reshape(3, 1))
    R_x, _ = cv2.Rodrigues(rot_vec1)

    rot_vec2 = np.ascontiguousarray((y_axis * np.radians(theta)).astype(np.float64).reshape(3, 1))
